...
public class OfflinePushInfo implements Serializable {
    ...
    public void disablePush(boolean disable) {
        if (disable) {
            setPushFlag(V2TIMOfflinePushInfo.OFFLINE_PUSH_FLAG_NO_PUSH);
        } else {
            setPushFlag(V2TIMOfflinePushInfo.OFFLINE_PUSH_FLAG_DEFAULT);
        }
    }
    ...
    public boolean isDisablePush() {
        if (getPushFlag() == V2TIMOfflinePushInfo.OFFLINE_PUSH_FLAG_NO_PUSH) {
            return true;
        } else {
            return false;
        }
    ...
    public void setIgnoreIOSBadge(boolean ignoreIOSBadge) {
        if (ignoreIOSBadge) {
            setBadgeMode(V2TIMOfflinePushInfo.OFFLINE_APNS_BADGE_MODE_IGNORE);
        } else {
            setBadgeMode(V2TIMOfflinePushInfo.OFFLINE_APNS_BADGE_MODE_DEFAULT);
        }
    }

    public boolean isIgnoreIOSBadge() {
        return badgeMode == V2TIMOfflinePushInfo.OFFLINE_APNS_BADGE_MODE_IGNORE;
    }
